...
%% Muse CSV format file data parser from NEW FIRMWARE (> Dec 2014)
clear all;
clc;
filename = 'thras_pain_2501';

% open file
fid = fopen(sprintf('MuseRec/csv/%s.csv',filename), 'r');
...
for i=1:size(line,2)-1
   ...
   switch data{2}
       ...
       case ' /muse/elements/blink'
           blink = [blink ; str2double(data{3})];
           blink_t = [blink_t; str2double(data{1})];
       case ' /muse/elements/experimental/concentration'
           conc = [conc ; str2double(data{3})];
           conc_t = [conc_t; str2double(data{1})];
       case ' /muse/config'
           config = line{i};
       case ' /muse/annotation'
           markers = [markers ; data{3}];
